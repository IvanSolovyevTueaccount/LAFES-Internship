...
#include "Timer_posix/timer_posix.h"
#include "Timer_posix/timer_stats.h"
...
{
	if (rtmGetErrorStatus(RT_MDL) == NULL && !rtmGetStopRequested(RT_MDL) && !StopRTA) {

		rtExtModePauseIfNeeded(rtmGetRTWExtModeInfo(RT_MDL), NUMST, (boolean_T *)&rtmGetStopRequested(RT_MDL));

		if (rtmGetStopRequested(RT_MDL)){
			printf("rtmGetStopRequested\n");
			return 1;
		}
...
# define EXT_MODE 0
    ...
    /* Initialization of Robocup real-time functionality */
    ...
    printf("Motion target v1.4, M.J.G. v.d. Molengraft & K.J. Meessen, 2012\n");
    ...
    timer_config.priority = prio;
    timer_config.sched_type = FIFO_SCHED;
    timer_config.timer_type = CONTINUOUS_CHECKING;
    timer_config.freq = Freq;
	timer_stat_t stats_struct;

	init_timer_statistics(&stats_struct, 1.0/Freq, (unsigned int)Freq);

	rtc_timer(user_apc, &timer_config, &stats_struct);
    ...
    print_timer_statistics(&stats_struct);
    ...
    save_timer_statistics(&stats_struct, filename);
#endif
